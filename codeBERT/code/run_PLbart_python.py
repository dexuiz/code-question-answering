...
MODEL_CLASSES = {'PLbart':(PLBartConfig, PLBartForConditionalGeneration,PLBartTokenizer)}
SRC_LANGUAGE = "python"
TGT_LANGUAGE = "en_XX"
...
def convert_examples_to_features(examples, tokenizer, args,stage=None):
    ...
    for example_index, example in enumerate(examples):
        ...
        with tokenizer.as_target_tokenizer():
                target_ids = tokenizer(example.target,padding="max_length", max_length=args.max_target_length, truncation=True).input_ids
...
def main():
    ...
    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,do_lower_case=args.do_lower_case,src_lang=SRC_LANGUAGE, tgt_lang=TGT_LANGUAGE)
    model = model_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path)
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for batch in bar:
                ...
                model_outputs =  model(input_ids=source_ids, attention_mask=source_mask, labels=target_ids)
                loss,logits = model_outputs.loss,model_outputs.logits
            ...
            if args.do_eval:
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        eval_model_outputs =  model(input_ids=source_ids, attention_mask=source_mask, labels=target_ids)    
                    
                    loss = eval_model_outputs.loss
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        preds = model.generate(input_ids=source_ids, attention_mask=source_mask,max_length=args.max_target_length,decoder_start_token_id=tokenizer.lang_code_to_id["en_XX"],num_beams=args.beam_size)
                        for pred in preds:
                        #     t=pred[0].cpu().numpy()
                        #     t=list(t)
                        #     if 0 in t:
